...
from utils.ScanNetV1_PlaneDataset import scannetv1_PlaneDataset
...
parser.add_argument('--cfg_path', default='configs/eval.yaml', type=str,
...
def eval(cfg, logger):
    ...
    if cfg.model.name == 'teacher':
        ...
        network = StudentNet(cfg)
    else:
        raise ('cannot find the network')
    ...
    with torch.no_grad():
        ...
        for iter, sample in enumerate(data_loader):
            ...
            # forward pass
            if cfg.model.name == 'teacher':
                t1,t2,t3, outputs = network(image)
            elif cfg.model.name == 'student':
                s1,s2,s3,outputs = network(image)
            else:
                raise ('cannot find the network')
            ...
            inferred_depth = inferred_depth * mask_pixelOnPlane.float() + pred_pixel_depth * (
                        1 - mask_pixelOnPlane.float())
        ...
        logger.info(
            f"RI(+):{plane_Seg_Metric[0]:.3f} | VI(-):{plane_Seg_Metric[1]:.3f} | SC(+):{plane_Seg_Metric[2]:.3f} | time_avg:{batch_time.avg:.3f}")
        logger.info("****************************************\n\n")
