...
class SetCriterion(nn.Module):
    ...
    # def loss_relative(self, outputs, targets, indices, num_planes,p_p=1.0,p_o = 0.5,p_c = 2.0):
    #     b,_,h,w = outputs['normal'].shape
    #
    #     assert b == len(targets)
    #     loss_parallel = 0.
    #     loss_ortho = 0.
    #     loss_contact = 0.
    #
    #     gt_normal = outputs['normal']
    #     pred_normal = outputs['segmentation_noraml']
    #
    #     plane_num = gt_normal.shape[0]
    #     loss_parallel += torch.sum(torch.abs(pred_normal * gt_normal - 1.0)**2)/plane_num
    #     loss_ortho += torch.sum(torch.abs(pred_normal * gt_normal)**2)/plane_num
    #     loss_contact += torch.sum(torch.abs(pred_normal - gt_normal))/plane_num
    #
    #     loss_relative = p_p*loss_parallel +p_c * loss_contact + p_o*loss_ortho
    #     losses_dict = {}
    #     losses_dict['loss_relative'] = loss_relative
    def loss_normal(self,outputs, targets, indices, num_planes_sum):
        gt_normal = outputs['noraml']
        pre_normal = outputs['pred_instance_normal']
        b,_,h,w = gt_normal.shape

        assert b==len(targets)
        loss_normal_l1 = torch.mean(torch.sum(torch.abs(gt_normal - pre_normal), dim=1))
        losses = {}
        losses['loss_normal'] = loss_normal_l1
    ...
    def get_loss(self,loss, output, target, indices, num_planes,**kwargs):
        loss_map = {
        ...
        assert loss in loss_map, f'loss is overflow the glass '
        return loss_map[loss](output,target,indices,num_planes,**kwargs)

    def forward(self,output,target):
        output_without_aux = {k:v for k,v in output.items() if k!= 'aux_outputs'}

        indices = self.matcher(output_without_aux,target)

        #calculate the 1th dim shape of target in all targets, and as a 1 size tensor
        num_planes = sum(tgt.shape[0] for tgt in target)
        num_planes = torch.as_tensor([num_planes],dtype=torch.float,device=next(iter(output.values())).device)
        num_planes = torch.clamp(num_planes ,min=1)


        #----------------------------compute losses
        losses = {}
        for loss in self.losses:
            losses.update(self.get_loss(loss, output, target, indices, num_planes))
