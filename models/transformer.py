...
class MLP(nn.Module):
    def __init__(self,input_dim,hidden_dim,out_dim,num_layers):
        ...
        self.layers = nn.ModuleList(nn.Linear(n,k) for n,k in zip([input_dim] + h,h+[out_dim]))
...
class Transformer(nn.Module):
    ...
    def forward(self, src, query_embed, pos_embed, tgt=None, src_lines=None, mask_lines=None, pos_embed_lines=None):
        ...
        if tgt is None:
            tgt = torch.zeros_like(query_embed)  # (1,20*bs,256)
        else:
        ...
        return hs.transpose(1, 2), None, memory.permute(1, 2, 0).view(bs, c, h, w)
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward_post(self, tgt, memory,
                     ...
                     pos: Optional[Tensor] = None,
                     query_pos: Optional[Tensor] = None):
        q = k = self.with_pos_embed(tgt, query_pos)  # tgt or tgt+query_pos
