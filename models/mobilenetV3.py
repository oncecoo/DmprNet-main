...
class MobileNetV3(nn.Module):
    def __init__(self, model_mode="LARGE", num_classes=100, multiplier=1.0, dropout_rate=0.0):
        ...
        self.block1 = []
        for in_channels, out_channels, kernal_size, stride, nonlinear, se, exp_size in layers1:
            in_channels = _make_divisible(in_channels * multiplier)
            out_channels = _make_divisible(out_channels * multiplier)
            exp_size = _make_divisible(exp_size * multiplier)
            self.block1.append(MobileBlock(in_channels, out_channels, kernal_size, stride, nonlinear, se, exp_size))
        self.block1 = nn.Sequential(*self.block1)

        self.block2 = []
        for in_channels, out_channels, kernal_size, stride, nonlinear, se, exp_size in layers2:
            in_channels = _make_divisible(in_channels * multiplier)
            out_channels = _make_divisible(out_channels * multiplier)
            exp_size = _make_divisible(exp_size * multiplier)
            self.block2.append(MobileBlock(in_channels, out_channels, kernal_size, stride, nonlinear, se, exp_size))
        self.block2 = nn.Sequential(*self.block2)

        self.block3 = []
        for in_channels, out_channels, kernal_size, stride, nonlinear, se, exp_size in layers3:
            in_channels = _make_divisible(in_channels * multiplier)
            out_channels = _make_divisible(out_channels * multiplier)
            exp_size = _make_divisible(exp_size * multiplier)
            self.block3.append(MobileBlock(in_channels, out_channels, kernal_size, stride, nonlinear, se, exp_size))
        self.block3 = nn.Sequential(*self.block3)

        self.block4 = []
        for in_channels, out_channels, kernal_size, stride, nonlinear, se, exp_size in layers4:
            in_channels = _make_divisible(in_channels * multiplier)
            out_channels = _make_divisible(out_channels * multiplier)
            exp_size = _make_divisible(exp_size * multiplier)
            self.block4.append(MobileBlock(in_channels, out_channels, kernal_size, stride, nonlinear, se, exp_size))
    ...
    def forward(self, x):
        ...
        out1 = self.init_conv(x)    #（b,16,1/2,1/2）
        out2 = self.block1(out1)
        out3 = self.block2(out2)
        out4 = self.block3(out3)     # (b,160,1/32,1/32)
        out5 = self.block4(out4)


        out6 = self.out_conv2(self.out_conv1(out5)) #(b,1000,1/32,1/32)   //// (b,1000,1/32,1/32)
        return out1,out2,out3,out4,out5,out6
#
# temp = torch.zeros((25, 3, 192, 256))
# model = MobileNetV3(model_mode="LARGE", num_classes=1000, multiplier=1.0)
#
# q,w,e,r,t,a = model(temp)
# print(q.shape,w.shape,e.shape,r.shape,t.shape,a.shape)
